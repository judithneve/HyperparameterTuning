...
start_seed <- job_id*100 + 20 # no overlap in seeds with any of the previous studies
set.seed(start_seed)
...
nrow_output <- nrow(hyperparameter_combinations)*2
out <- data.frame(
  ...
  mtry            = rep(NA,             nrow_output),
  min.node.size   = rep(NA,             nrow_output),
  replace         = rep(NA,             nrow_output),
  sample.fraction = rep(NA,             nrow_output),
  splitrule       = rep(NA,             nrow_output)
)
row_out <- 0
...
# sensitivity analysis: we also compare optimising for deviance and optimising for AUC
for (metric in c("Deviance", "AUC")) {
  for (combination in 1:nrow(hyperparameter_combinations)) {
    ...
    if (metric == "AUC") {
      ...
      start_tune <- Sys.time()
      mod <- tune_hyperparameters_AUC(dat, tunegrid, sample.fraction_candidates, replace_candidates, ctrl)
      end_tune   <- Sys.time()
      print(end_tune)
    }
    
    tuning_time <- difftime(end_tune, start_tune, units = "mins")
    out[row_out,"mtry"] <- mod$model$bestTune$mtry
    out[row_out,"min.node.size"] <- mod$model$bestTune$min.node.size
    out[row_out,"splitrule"] <- mod$model$bestTune$splitrule %>% as.character()
    out[row_out,"sample.fraction"] <- mod$model$dots$sample.fraction
    out[row_out,"replace"] <- mod$model$dots$replace
    ...
    out[row_out,12:13] <- perf[1:2]
  }
}

##### Save #####

filename <- paste0("Study4/Data/sim/study4_run", job_id, ".rds")
