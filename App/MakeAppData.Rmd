...
```{r}
df1 <- list.files(path = "./../Study1/Data/perfs/", pattern = ".rds") %>%
  paste0("./../Study1/Data/perfs/", .) %>% 
  map(readRDS) %>% 
  bind_rows() %>% 
...
df2 <- list.files(path = "./../Study2/Data/perfs/", pattern = ".rds") %>%
  paste0("./../Study2/Data/perfs/", .) %>% 
  map(readRDS) %>% 
  bind_rows() %>% 
...
df3 <- list.files(path = "./../Study3/Data/perfs/", pattern = ".rds") %>%
  paste0("./../Study3/Data/perfs/", .) %>% 
  map(readRDS) %>% 
  bind_rows() %>% 
  mutate(time = time*60) %>% 
  group_by(n_prop, p, EF) %>% 
