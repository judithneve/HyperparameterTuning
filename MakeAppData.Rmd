...
scale_color_mine <- function(study) {
  ...
  if (study == "Study2") {
    ...
    names(opt.labs) <- c("Calibration slope", "Calibration intercept", "Brier score", "Logarithmic loss", "AUC", "Classification accuracy", "Cohen's Kappa", "Deviance")
    opt.labs <- opt.labs[c(8, 5, 4, 6, 7, 3, 2, 1)]
    metric_pal <- viridis::inferno(n = 8)
...
```{r}
save(dat, df_scale_setter, df_targets, Metric.labs, scale_color_mine, scale_shape_mine, HP.labs, opt.labs,
     file = "appdata.RData")
```
