...
expected_seeds <- c((1:3000)*100 + 8, (1:1500)*100 + 16)
expected_seeds[!(expected_seeds %in% unique(df$start_seed))] %>% paste(collapse = ", ")
(expected_seeds[!(expected_seeds %in% unique(df$start_seed))]/100) %>% floor() %>% paste(collapse = ",")
...
```{r}
df_time <- list.files(path = "./Data/sim/archive/", pattern = ".rds")[-1] %>%
  paste0("./Data/sim/archive/", .) %>% 
  map(readRDS) %>% 
  bind_rows()
...
```{r}
seed_to_scenario <- list.files(path = "./Data/sim/", pattern = ".rds") %>%
  paste0("./Data/sim/", .) %>% 
  map(readRDS) %>% 
  bind_rows()
seed_to_scenario <- seed_to_scenario %>% 
  select(start_seed, p, EF, n, n_prop) %>% 
  unique() %>% 
  mutate(scenario = as.numeric(factor(paste(p, EF, n_prop))))           # TODO: NICER SCENARIO NUMBERING
predictions <- list.files(path = "./Data/preds/", pattern = ".rds") %>%
  paste0("./Data/preds/", .)
predictions <- predictions[1:(length(predictions)/2)]
scenario_pal <- viridis::viridis(n = 12)
...
for (pred in predictions) {
  selected <- sample(c(rep(T, 1e4), rep(F, 1e5-1e4)), 1e5) %>% rep(9)
  temp <- readRDS(pred) %>% 
    mutate(obs = ifelse(obs == "neg", 0, 1),
           selected = selected) %>% 
    filter(selected)
  col_temp <- seed_to_scenario %>% 
    filter(start_seed == unique(temp$start_seed)) %>% 
    pull(EF)
  plot2 <- plot2 +
    geom_line(data = temp,
              aes(x = prob, y = obs),
              col = scenario_pal[col_temp*10],
              stat = "smooth",
              method = stats::loess, 
              formula = y ~ x,
              se = F, 
...
ggsave("test_calplot_ef.png", plot2, width = 15, height = 15, units = "in")
end_plot2 <- Sys.time()
```
