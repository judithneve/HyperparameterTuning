...
```{r}
dat <- sim_data(scenarios, betas_matrix[[1]], 1) # 1000 reps: 7GB - if stored as a list, 4GB
```
...
```{r}
mod <- glm(Y ~ . + X1:X4 + X2:X5, data = toy_dat %>% dplyr::select(-c(id, sample_size_prop, n_pred, event_fraction, dataset_id)), family = binomial())
# AUC and event faction also work?
summary(mod)
```
...
mod <- ranger(as.factor(Y) ~ X1 + X2 + X3 + X4 + X5 + X6 + X7 + X8, data = toy_dat, mtry = 2, splitrule = "hellinger", min.node.size = 1, probability = TRUE)

val_dat <- dat[dat[,"dataset_id"] == "1_1",] %>%
  as.data.frame() %>% 
  pivot_wider(names_from = Pred_number, values_from = Pred_value)

val_dat[c(1, 7:14)]  <- lapply(val_dat[c(1, 7:14)], as.numeric)

val_dat$Y %>% mean()

pred <- predict(mod, data = val_dat, type = "p")
